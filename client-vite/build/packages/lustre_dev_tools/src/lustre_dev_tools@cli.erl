...
-opaque cli(XWJ) :: {cli,
        fun((env()) -> {env(),
            {ok, XWJ} | {error, lustre_dev_tools@error:error()}})}.
...
-spec run(cli(XWK), glint:flags()) -> {ok, XWK} |
...
-spec from_result({ok, XWS} | {error, lustre_dev_tools@error:error()}) -> cli(XWS).
from_result(Result) ->
    {cli, fun(Env) -> {Env, Result} end}.

-file("/home/runner/work/dev-tools/dev-tools/src/lustre_dev_tools/cli.gleam", 88).
-spec do(cli(XWW), fun((XWW) -> cli(XWY))) -> cli(XWY).
do(Step, Next) ->
    {cli,
        fun(Env) ->
            {Env@1, Result} = (erlang:element(2, Step))(Env),
...
-spec 'try'(
    {ok, XXH} | {error, lustre_dev_tools@error:error()},
    fun((XXH) -> cli(XXK))
) -> cli(XXK).
'try'(Result, Next) ->
...
-spec log(binary(), fun(() -> cli(XXN))) -> cli(XXN).
log(Message, Next) ->
    {cli,
        fun(Env) ->
...
-spec success(binary(), fun(() -> cli(XXQ))) -> cli(XXQ).
...
-spec notify(binary(), fun(() -> cli(XXT))) -> cli(XXT).
notify(Message, Next) ->
    {cli,
        fun(Env) ->
            Env@1 = erlang:setelement(3, Env, case erlang:element(3, Env) of
...
-spec template(binary(), fun((binary()) -> cli(XXY))) -> cli(XXY).
template(Name, Next) ->
    {cli,
        fun(Env) ->
...
-spec get_config_value(
    ...
    XYE,
    list(binary()),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, XYE} |
        {error, any()}),
    fun((glint:flags()) -> {ok, XYE} | {error, any()})
) -> cli(XYE).
