...
-spec do_keyed(lustre@internals@vdom:element(PAQ), binary()) -> lustre@internals@vdom:element(PAQ).
do_keyed(El, Key) ->
    case El of
...
-spec keyed(
    fun((list(lustre@internals@vdom:element(PAJ))) -> lustre@internals@vdom:element(PAJ)),
    list({binary(), lustre@internals@vdom:element(PAJ)})
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(PAT)),
    list(lustre@internals@vdom:element(PAT))
) -> lustre@internals@vdom:element(PAT).
...
-spec advanced(
    binary(),
    binary(),
    list(lustre@internals@vdom:attribute(PAZ)),
    list(lustre@internals@vdom:element(PAZ)),
    boolean(),
    boolean()
) -> lustre@internals@vdom:element(PAZ).
...
-spec fragment(list(lustre@internals@vdom:element(PBJ))) -> lustre@internals@vdom:element(PBJ).
fragment(Elements) ->
    element(
...
-spec map(lustre@internals@vdom:element(PBN), fun((PBN) -> PBP)) -> lustre@internals@vdom:element(PBP).
map(Element, F) ->
    case Element of
        {text, Content} ->
            {text, Content};
...
-spec get_root(fun((fun((PBR) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> lustre@effect:effect(PBR).
get_root(Effect) ->
