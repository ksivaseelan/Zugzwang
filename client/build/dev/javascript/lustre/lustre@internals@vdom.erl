...
-type attribute(OFV) :: {attribute,
        binary(),
        gleam@dynamic:dynamic_(),
        boolean()} |
    {event,
        binary(),
        fun((gleam@dynamic:dynamic_()) -> {ok, OFV} |
            {error, list(gleam@dynamic:decode_error())})}.

-file("/Users/hayleigh/work/lustre-labs/lustre/src/lustre/internals/vdom.gleam", 508).
-spec attribute_to_event_handler(attribute(OHX)) -> {ok,
        {binary(),
            fun((gleam@dynamic:dynamic_()) -> {ok, OHX} |
                {error, list(gleam@dynamic:decode_error())})}} |
    {error, nil}.
...
-spec do_element_list_handlers(
    list(element(OGJ)),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, OGJ} |
        {error, list(gleam@dynamic:decode_error())})),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, OGJ} |
    {error, list(gleam@dynamic:decode_error())})).
...
-spec do_handlers(
    element(OGB),
    gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, OGB} |
        {error, list(gleam@dynamic:decode_error())})),
    binary()
) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok, OGB} |
    {error, list(gleam@dynamic:decode_error())})).
...
-spec handlers(element(OFW)) -> gleam@dict:dict(binary(), fun((gleam@dynamic:dynamic_()) -> {ok,
        OFW} |
    {error, list(gleam@dynamic:decode_error())})).
handlers(Element) ->
