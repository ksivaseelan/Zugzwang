...
-spec component(list(lustre@internals@vdom:attribute(SYG))) -> lustre@internals@vdom:element(SYG).
component(Attrs) ->
...
-spec subscribe(binary(), fun((lustre@internals@patch:patch(SYT)) -> nil)) -> lustre@internals@runtime:action(SYT, lustre:server_component()).
...
-spec do_select(
    fun((fun((SZH) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(SZH))
) -> lustre@effect:effect(SZH).
do_select(Sel) ->
...
-spec select(
    fun((fun((SZC) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(SZC))
) -> lustre@effect:effect(SZC).
select(Sel) ->
    do_select(Sel).

-file("/Users/hayleigh/work/lustre-labs/lustre/src/lustre/server_component.gleam", 257).
-spec set_selector(
    gleam@erlang@process:selector(lustre@internals@runtime:action(any(), SZN))
