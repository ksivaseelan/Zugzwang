...
var ErrorPage = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
var UserSelectedPieces = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserStartedGame = class extends CustomType {
};
var ApiReturnedGame = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
var GameCreateResponse = class extends CustomType {
...
function get_route2() {
  let uri = (() => {
    let $2 = (() => {
      let _pipe2 = get_route();
      return parse2(_pipe2);
    })();
    if ($2.isOk()) {
      let uri2 = $2[0];
      return uri2;
    } else {
      throw makeError("panic", "client", 37, "get_route", "Invalid URI", {});
    }
  })();
  let _pipe = uri.path;
  let _pipe$1 = inspect2(_pipe);
...
function init3(_) {
  return [new Model2("", "", get_route2()), init2(on_url_change)];
}
function start_game(model) {
  let decoder = decode3(
    (var0, var1, var2) => {
      return new GameCreateResponse(var0, var1, var2);
    },
    field("pid", string),
    field("room", string),
    field("color", string)
  ...
  let expect = expect_json(
    decoder,
    (var0) => {
      return new ApiReturnedGame(var0);
    }
  );
  let object3 = object2(
    toList([
  ...
  return post("http://localhost:6969/create", object3, expect);
}
function update(model, msg) {
  if (msg instanceof UserSelectedPieces && msg[0] instanceof Black) {
    return [model.withFields({ color: "black" }), none()];
  } else if (msg instanceof UserSelectedPieces && msg[0] instanceof White) {
    return [model.withFields({ color: "white" }), none()];
    ...
    let error = msg[0][0];
    return [model.withFields({ route: new ErrorPage(error) }), none()];
  } else {
    let route = msg[0];
...
function view(model) {
  ...
  if ($ instanceof Home) {
    return body(
      toList([]),
      toList([h1(toList([]), toList([text("Welcome to Zugzwang")]))])
    );
